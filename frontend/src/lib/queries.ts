...
export const authApi = {
  ...
  verifyEmail: (token: string) =>
    api.post<{ data: { user: User; detail: string } }>('/auth/verify-email/', { token }).then(r => r.data.data),

  resendVerification: () =>
    api.post<{ data: { detail: string } }>('/auth/resend-verification/').then(r => r.data.data),
};
...
export const resumeApi = {
  ...
  // V2 — Word export (blob response is handled directly in component)
  exportWordUrl: (id: string) => `/resumes/${id}/export-word/`,
};
