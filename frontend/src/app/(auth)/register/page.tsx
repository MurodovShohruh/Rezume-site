...
const schema = z.object({
  full_name: z.string().min(2, 'Ism kamida 2 ta harf bo\'lsin'),
...
const benefits = [
  'AI bilan professional matn',
  '5 ta chiroyli shablon',
  'PDF bir click bilan',
  'Bepul boshlang',
];
...
export default function RegisterPage() {
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      login(result);
      toast.success('Xush kelibsiz! Birinchi rezumengizni yarating 🎉');
    ...
    <div className="min-h-screen bg-ink-950 flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-sm animate-fade-up">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-ink-400 mt-2 text-sm">Bepul hisob yarating</p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 mb-6">
          {benefits.map((b, i) => (
            <div key={i} className="flex items-center gap-2 text-ink-400 text-xs">
              <Check size={13} className="text-sage-400 flex-shrink-0" />
              {b}
            </div>
          ))}
        </div>
        ...
        <div className="bg-ink-900 border border-ink-700 rounded-2xl p-8 shadow-xl shadow-ink-950/50">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            ...
            <div>
              <label className="block text-sm font-medium text-ink-300 mb-2">To'liq ism</label>
              <input
                {...register('full_name')}
                type="text"
                placeholder="Ali Valiyev"
                className="w-full bg-ink-800 border border-ink-600 text-ink-100 placeholder:text-ink-600
                  rounded-lg px-4 py-3 text-sm focus:outline-none focus:border-amber-500
